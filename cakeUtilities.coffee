...
execute = ->
  # equal of command line
	# $ "browserify --debug -t coffeeify ./src/main.coffee > bundle.js "
  b = browserify()
  ...
  , (err, result) ->
    if not err
      fs.writeFile 'public/bundle.js', result, (err) ->
        if not err
...
watchDirectory = (directory, callback) ->
	readdirp root: directory, fileFilter: '*.coffee'
	.on 'data', (entry) ->
		fs.watchFile entry.fullPath, (curr, prev) ->
